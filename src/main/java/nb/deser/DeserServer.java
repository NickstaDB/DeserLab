...
public class DeserServer {
  ...
  public void run(String listenAddress, int listenPort) throws Exception {
    ...
    System.out.println(
        "[+] DeserServer started, listening on " + serverSock.getInetAddress().getHostAddress()
    ...
    while (true) {
      try {
        ...
        System.out.println(
            "[+] Connection accepted from " + clientSock.getInetAddress().getHostAddress() + ":"
                + clientSock.getPort());
        ...
        if (ois.readShort() > PROTOCOL_VERSION) {
          System.out.println(
              "[-] Client protocol version is greater than server protocol version, killing connection...");
          clientSock.close();
          continue;
        }
